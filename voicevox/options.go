...
//
// **数詞と固有名詞のためにあります。** 読み変換は数字をそのまま通し、VOICEVOX は
// それを字面どおりに読むため、助数詞が付いた数字は不自然になります（8日→ハチニチ、
// 1人→イチニン、20歳→ニジュッサイ）。日付や人数を含む台本では、ここで読みを与えるのが
// 唯一の直し方です。プロンプトで「カタカナで書かせる」のは台本そのものを歪めますし、
// 変換結果の確認では数字のまま見えるため、合成するまで誤読に気づけません。
//
